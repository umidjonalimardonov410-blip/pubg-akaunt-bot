...
After deployment `71396c32-9e8c-4eac-bff7-31c6babec934` reached `SUCCESS` on commit `2be9749550b1eaab2ff0d9811273108a9f8793fe`, the public `/accounts` route loaded successfully. The live header now shows only Bozor, Saqlanganlar, Sotish, Kafolatli savdo, Referral, Yordam, and Kabinet; deprecated Pro navigation is absent. The live controls expose `3 ta ustunli ko'rinish` and `5 qatorli ixcham ko'rinish`. The marketplace query was still showing `Akkauntlar yuklanmoqda...` during the browser capture, so account-data rendering itself needs a separate authenticated/data-availability check.

Interactive live verification after the static-path fix succeeded: clicking `5 qatorli ixcham ko'rinish` rendered full compact rows with Level, K/D, Win rate, games, Skins, and Price for the four visible accounts; clicking `3 ta ustunli ko'rinish` returned the compact browse cards with Level, region, K/D, and Price. The live response contains no Pro menu items. Public source now reflects the intended layout behavior.

Final root-route check succeeded after deployment `71396c32-9e8c-4eac-bff7-31c6babec934`: `https://inferno-stealth-production.up.railway.app/` now serves the Inferno Stealth SPA with the marketplace header, hero, account cards, and no `Not Found` response.
