...
export function TelegramBotCommandSection() {
  ...
  return <Card className="border-red-500/20 bg-red-500/[0.04] text-card-foreground"><CardHeader><CardTitle className="flex items-center gap-2 text-red-300"><Bot className="h-5 w-5" /> Telegram bot komandalar</CardTitle></CardHeader><CardContent className="space-y-4"><p className="text-sm leading-6 text-muted-foreground">Quyidagi komandalar Telegram bot menyusida foydalanuvchini to‘g‘ri Mini App bo‘limiga olib kiradi.</p><div className="flex flex-wrap gap-2">{Object.keys(commands).map((command) => <Button key={command} size="sm" variant={selected === command ? 'default' : 'outline'} onClick={() => { setSelected(command); setSent(false); }}>{command}</Button>)}</div><div className="rounded-xl border border-white/10 bg-black/20 p-4"><p className="font-semibold text-foreground">{commands[selected].title}</p><p className="mt-2 text-sm text-muted-foreground">{commands[selected].text}</p></div><div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => setShowWebhook(!showWebhook)}>{showWebhook ? 'Webhook previewni yopish' : 'Webhook previewni ko‘rish'}</Button>{showWebhook && <Button onClick={() => { setSent(true); toast.success('Webhook simulyatsiyasi muvaffaqiyatli yuborildi'); }}>Test webhook yuborish</Button>}</div>{showWebhook && <div className="rounded-xl border border-red-300/20 bg-black/30 p-4"><div className="flex items-center justify-between gap-3"><p className="text-xs font-bold uppercase tracking-wider text-red-200">POST /api/telegram/webhook</p><Badge>{sent ? '200 OK' : 'Preview'}</Badge></div><pre className="mt-3 max-h-56 overflow-auto whitespace-pre-wrap text-[11px] leading-5 text-white/60">{payload}</pre></div>}<p className="text-xs text-muted-foreground">Production webhook uchun BotFather tokeni va webhook domeni ulanadi; bu karta esa buyruq oqimi va Telegram update payloadini Mini App ichida xavfsiz ko‘rsatadi.</p></CardContent></Card>;
}
