...
export function FavoriteButton({ accountId, compact = false }: { accountId: number; compact?: boolean }) {
  ...
  return <button type="button" onPointerDown={event => event.stopPropagation()} onClick={event => { event.stopPropagation(); handleClick(); }} disabled={toggle.isPending} className={`heart-button ${saved ? 'is-saved' : ''} ${compact ? 'h-7 w-7 rounded-lg touch-manipulation' : 'h-11 w-11 rounded-xl'} grid shrink-0 place-items-center border transition duration-200 active:scale-90 ${saved ? 'border-amber-300/50 bg-amber-400/20 text-amber-200 shadow-[0_0_18px_rgba(245,197,66,.18)]' : 'border-white/10 bg-black/30 text-white/65 hover:border-amber-300/40 hover:text-white'}`} aria-label={saved ? 'Saqlanganlardan olib tashlash' : 'Saqlash'} aria-pressed={saved}><Heart className={`heart-icon ${compact ? 'h-3 w-3' : 'h-5 w-5'} ${saved ? 'fill-current' : ''}`} /><span aria-hidden className="heart-burst" /></button>;
}
...
export function SavedPage({ onNavigate }: { onNavigate: (path: string) => void }) {
  ...
  if (!isAuthenticated) return <main className="mx-auto max-w-2xl rounded-3xl border border-amber-400/20 bg-[#0e1013] p-4 sm:p-5 sm:p-8 text-center"><Heart className="mx-auto h-10 w-10 text-amber-200" /><h1 className="mt-4 font-display text-xl sm:text-2xl font-black text-white">Saqlangan akkauntlar</h1><p className="mt-2 text-sm leading-6 text-white/45">Qiziqqan akkauntlaringizni saqlash uchun profilga kiring.</p><Button onClick={() => onNavigate('/profile')}>Profilga kirish <ArrowRight className="h-4 w-4" /></Button></main>;
  const accounts = (query.data ?? []).map(normalize);
  return <main className="space-y-4 pb-24 sm:space-y-6 lg:pb-6"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><span className="text-[10px] font-bold uppercase tracking-[0.22em] text-amber-300">Watchlist</span><h1 className="mt-2 font-display text-2xl sm:text-3xl font-black text-white">Saqlangan akkauntlar</h1><p className="mt-2 text-sm text-white/45">Qayta ko‘rib chiqmoqchi bo‘lgan e’lonlaringiz bir joyda.</p></div><Button ghost onClick={() => onNavigate('/accounts')}><ArrowLeft className="h-4 w-4" />Bozorga qaytish</Button></div>{query.isLoading ? <div className="card-glow rounded-2xl border border-white/[0.08] bg-[#0e1013] p-4 sm:p-5 sm:p-8 text-center text-sm text-white/45">Saqlanganlar yuklanmoqda...</div> : accounts.length === 0 ? <div className="rounded-2xl border border-dashed border-white/10 bg-[#0e1013] p-10 text-center"><Heart className="mx-auto h-8 w-8 text-white/20" /><h2 className="mt-4 font-display text-lg font-black text-white">Ro‘yxat hozircha bo‘sh</h2><p className="mt-2 text-sm text-white/40">Bozorda yurakcha tugmasini bosib akkaunt saqlang.</p><Button className="mt-5" onClick={() => onNavigate('/accounts')}>Bozorni ko‘rish</Button></div> : <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{accounts.map(account => <article key={account.id} className="overflow-hidden card-glow rounded-2xl border border-white/[0.08] bg-[#0e1013]"><img src={account.image} alt={account.playerName} loading="lazy" decoding="async" className="h-44 w-full img-live object-cover" /><div className="p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-wider text-amber-200">{account.tag}</p><h2 className="mt-1 font-display text-lg font-black text-white">{account.playerName}</h2><p className="mt-1 text-xs text-white/40">LVL {account.level} · {account.rank} · {account.region}</p></div><FavoriteButton accountId={account.id} compact /></div><p className="mt-3 line-clamp-2 text-xs leading-5 text-white/45">{account.description}</p><div className="mt-4 flex items-center justify-between"><span className="font-display text-lg font-black text-amber-200">{uzNumber(account.price)} so‘m</span><Button onClick={() => onNavigate(`/account/${account.id}`)}>Batafsil <ArrowRight className="h-4 w-4" /></Button></div></div></article>)}</div>}</main>;
}
...
export function ChatPage({ id, onBack }: { id: number; onBack: () => void }) {
  ...
  if (!isAuthenticated) return <main className="mx-auto max-w-2xl rounded-3xl border border-amber-400/20 bg-[#0e1013] p-4 sm:p-5 sm:p-8 text-center"><MessageCircle className="mx-auto h-10 w-10 text-amber-200" /><h1 className="mt-4 font-display text-xl sm:text-2xl font-black text-white">Xavfsiz chat</h1><p className="mt-2 text-sm text-white/45">Sotuvchi bilan yozishish uchun tizimga kiring.</p></main>;
  const messages = query.data ?? [];
  return <main className="mx-auto max-w-3xl space-y-5"><button onClick={onBack} className="inline-flex items-center gap-2 text-sm font-bold text-white/45 hover:text-white"><ArrowLeft className="h-4 w-4" />Orqaga</button><section className="overflow-hidden rounded-3xl border border-white/[0.08] bg-[#0e1013]"><div className="flex items-center gap-3 border-b border-white/[0.08] p-5"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-amber-400/10 text-amber-200"><MessageCircle className="h-5 w-5" /></span><div><h1 className="font-display text-xl font-black text-white">Inferno xavfsiz chat</h1><p className="mt-1 text-xs text-white/40">Login va parolni chatga yozmang. Faqat savdo tafsilotlarini muhokama qiling.</p></div><Shield className="ml-auto h-5 w-5 text-emerald-300" /></div><div className="min-h-80 space-y-3 p-5">{query.isLoading ? <p className="text-sm text-white/40">Xabarlar yuklanmoqda...</p> : messages.length === 0 ? <div className="grid min-h-64 place-items-center text-center"><MessageCircle className="h-8 w-8 text-white/20" /><p className="mt-3 text-sm text-white/40">Birinchi xabarni yuboring.</p></div> : messages.map(message => <div key={message.id} className={`flex ${message.senderId === user?.id ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[82%] rounded-2xl px-4 py-3 text-sm leading-6 ${message.senderId === user?.id ? 'bg-amber-400 text-black' : 'border border-white/10 bg-white/[0.04] text-white/75'}`}><p>{message.body}</p><span className="mt-1 block text-[10px] opacity-55">{new Date(message.createdAt).toLocaleString('uz-UZ')}</span></div></div>)}</div><form onSubmit={event => { event.preventDefault(); if (body.trim()) send.mutate({ threadId: id, body: body.trim() }); }} className="flex gap-2 border-t border-white/[0.08] p-4"><input className="field-input flex-1" value={body} onChange={event => setBody(event.target.value)} placeholder="Xabar yozing..." maxLength={2000} /><Button disabled={send.isPending || !body.trim()}>{send.isPending ? '...' : <Send className="h-4 w-4" />}Yuborish</Button></form></section></main>;
}
...
export function ReferralPage() {
  ...
  if (!isAuthenticated) return <main className="mx-auto max-w-2xl rounded-3xl border border-amber-400/20 bg-[#0e1013] p-4 sm:p-5 sm:p-8 text-center"><Star className="mx-auto h-10 w-10 text-amber-200" /><h1 className="mt-4 font-display text-xl sm:text-2xl font-black text-white">Referral dasturi</h1><p className="mt-2 text-sm text-white/45">Do‘stlaringizni taklif qilish va bonus olish uchun tizimga kiring.</p></main>;
