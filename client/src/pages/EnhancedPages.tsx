...
export function FavoriteButton({ accountId, compact = false }: { accountId: number; compact?: boolean }) {
  ...
  return <button type="button" onPointerDown={event => event.stopPropagation()} onClick={event => { event.stopPropagation(); handleClick(); }} disabled={toggle.isPending} className={`${compact ? 'h-7 w-7 rounded-lg touch-manipulation' : 'h-11 w-11 rounded-xl'} grid shrink-0 place-items-center border transition duration-200 active:scale-90 ${saved ? 'border-amber-300/50 bg-amber-400/20 text-amber-200 shadow-[0_0_18px_rgba(245,197,66,.18)]' : 'border-white/10 bg-black/30 text-white/65 hover:border-amber-300/40 hover:text-white'}`} aria-label={saved ? 'Saqlanganlardan olib tashlash' : 'Saqlash'} aria-pressed={saved}><Heart className={`${compact ? 'h-3 w-3' : 'h-5 w-5'} transition duration-200 ${saved ? 'fill-current scale-110' : ''}`} /></button>;
}
